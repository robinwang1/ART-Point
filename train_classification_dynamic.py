...
class CWFGSM(object):
    ...
    def forward(self, target_cls, points, labels):
        ...
        return adversarial_examples, delta
...
def main(args):
    ...
    #                                              num_workers=16)
    # new data_sets!!!!
    train_points = np.load('train1024point.npy')
    train_labels = np.load('train1024label.npy').flatten()
    print('train data size:', train_labels.shape)
    test_points = np.load('test1024point.npy')
    test_labels = np.load('test1024label.npy').flatten()
    ...
    # 1 for eval, 2 for train
    if not args.use_cpu:
        classifier_eval = model.get_model(num_class, normal_channel=args.use_normals)
        classifier_train = model.get_model(num_class, normal_channel=args.use_normals)
        classifier_eval = classifier_eval.cuda()
        ...
        classifier_train = model.get_model(num_class, normal_channel=args.use_normals)
        criterion = model.get_loss()
    classifier_eval.apply(inplace_relu)
    classifier_train.apply(inplace_relu)

    pretrained_dir = Path('./pretrained_models/modelnet40/pn1.pth')
    checkpoint_pre = torch.load(pretrained_dir)
    # start_epoch = checkpoint['epoch']
    classifier_eval.load_state_dict(checkpoint_pre['model_state_dict'])
    ...
    if args.scales:
        print('scale data with %.2f' % args.scales)
    for epoch in range(start_epoch, args.epoch):
        ...
        attack_num = 0
        attack_correct = 0
        log_string('eval:{}'.format(classifier_eval.state_dict()['fc1.weight']))
        log_string('train:{}'.format(classifier_train.state_dict()['fc1.weight']))
        if epoch > 0:
            modelsavepath = str(checkpoints_dir) + '/best_model.pth'
            checkpoint = torch.load(modelsavepath)
            classifier_eval.load_state_dict(checkpoint['model_state_dict'])
            classifier_train.load_state_dict(checkpoint['model_state_dict'])
            log_string('eval:{}'.format(classifier_eval.state_dict()['fc1.weight']))
            log_string('train:{}'.format(classifier_train.state_dict()['fc1.weight']))
        ...
        if not args.rp:
            ...
            adv_dataset = torch.utils.data.TensorDataset(torch.from_numpy(adv_samples),
                                                         torch.from_numpy(adv_labels))  # create your datset
            adv_dataloader = torch.utils.data.DataLoader(adv_dataset, batch_size=17,
        ...
        if args.optimizer == 'Adam':
            ...
            optimizer = torch.optim.SGD(classifier_train.parameters(), lr=0.01, momentum=0.9)
        ...
        for epoch_adv in range(args.inner_epoch):
            ...
            for batch_id_min, (points_adv, target_adv) in tqdm(enumerate(adv_dataloader, 0), total=len(adv_dataloader),
                ...
                global_step += 1
            scheduler.step()
            log_string('{} {} optim: {}'.format(epoch, epoch_adv, optimizer.param_groups[0]['lr']))
        ...
        global_epoch += 1
        logger.info('End of Max Step %d...' % epoch)
